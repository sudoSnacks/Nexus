    ...
    return (
        <div className="min-h-screen text-gray-900 font-sans bg-white selection:bg-blue-500 selection:text-white">
            ...
            <header className="fixed top-0 left-0 right-0 z-50 p-6 flex justify-between items-center border-b border-black/5 backdrop-blur-md bg-white/60 shadow-sm">
                <Link href="/test-landing" className="flex items-center justify-center w-10 h-10 rounded-full bg-white hover:bg-gray-100 border border-black/10 transition-all font-bold text-black shadow-sm group">
                ...
                {user ? (
                    <div className="flex items-center gap-2 md:gap-4">
                        ...
                        {isUserAdmin && (
                            <Link
                                href="/events/new"
                                className="bg-white hover:bg-gray-50 border border-black/10 text-black px-2 py-1 text-xs md:text-base md:px-4 md:py-2 rounded-lg transition-all shadow-sm hover:shadow-md font-medium"
                            >
                                + Create Event
                            </Link>
                        )}
                    ...
                    <Link href="/login" className="bg-[#1e1e1e] hover:bg-black text-white font-medium py-2 px-4 rounded-lg transition-colors text-sm shadow-md">
                        Sign in
                    </Link>
                )
                }
